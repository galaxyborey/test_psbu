...
<div class="right_col" role="main">
    <div class="row">
        <div class="col-md-12">
          <div class="x_panel">
            <div class="x_title">
              <h2>{{trans('lang.edit_match')}}</h2>
              <div class="clearfix"></div>
            </div>
            <div class="x_content">
              <br>
                  {!! Form::open(['route' => ['matchs.update', $match->id], 'method' => 'PUT']) !!}
                  <input type="hidden" name="date_id" value="{{$match->date_id}}" >
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                    ...
                    <select class="form-control" name="league_id">
                      ...
                      {{\App\Http\Controllers\HomeController::getLeagueList($match->league_id)}}
                    </select>
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                      {{ Form::label('match_date', trans('lang.match_date'),['class' => 'col-md-3 col-sm-6 col-xs-12']) }}
                      {{ Form::text('match_date', date('d/m/Y',strtotime($match->date_match)), ['class' => 'form-control datesport', 'id' => 'match_date']) }}
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                    {{ Form::label('leauge_name', trans('lang.first_club'),['class' => 'col-md-3 col-sm-6 col-xs-12']) }}
                    <select class="form-control" name="club_left">
                      <option value="0">{{trans('lang.please_selete')}} </option>
                      {{\App\Http\Controllers\HomeController::getClubList($match->f_teamid)}}
                    </select>
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                    ...
                    <select class="form-control" name="club_rigth">
                      <option value="0">{{trans('lang.please_selete')}} </option>
                      {{\App\Http\Controllers\HomeController::getClubList($match->s_teamid)}}
                    </select>
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                      {{ Form::label('time_match', trans('lang.time_match'),['class' => 'col-md-6 col-sm-6 col-xs-12']) }}
                      {{ Form::text('time_match', $match->time_match, ['class' => 'form-control', 'id' => 'time_match']) }}
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                      {{ Form::label('score', trans('lang.score'),['class' => 'col-md-6 col-sm-6 col-xs-12']) }}
                      {{ Form::text('score', $match->score, ['class' => 'form-control', 'id' => 'score','placeholder'=>' 0 - 0 ']) }}
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                      {{ Form::label('play_location', trans('lang.play_location'),['class' => 'col-md-6 col-sm-6 col-xs-12']) }}
                      {{ Form::text('play_location', $match->play_location, ['class' => 'form-control', 'id' => 'play_location']) }}
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                      {{ Form::label('on_active', trans('lang.on_active'),['class' => 'col-md-6 col-sm-6 col-xs-12']) }}
                      {{ Form::text('on_active', $match->active_main, ['class' => 'form-control', 'id' => 'on_active']) }}
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                      {{ Form::label('tv_live', trans('lang.tv_live'),['class' => 'col-md-3 col-sm-6 col-xs-12']) }}
                      {{ Form::text('tv_live', $match->tv_live, ['class' => 'form-control', 'id' => 'tv_live']) }}
                  </div>
                  <div class="form-group col-md-6 col-sm-6 col-xs-12">
                    ...
                    <div class="col-md-10 col-sm-10 col-xs-10">
                      <input id="thumbnail" class="form-control" type="text" value="{{$match->tv_img}}" name="filepath">
                    </div>
                   </div>
                   <img id="holder" src="{{url('')}}/{{$match->tv_img}}" style="margin-top:15px;max-height:100px;">
                  <div class="form-group">
                      {{ Form::label('slug', trans('lang.match_url')) }}
                      {{ Form::text('slug', $match->slug, ['class' => 'form-control', 'id' => 'slug']) }}
                  </div>
                    <div class="form-group">
