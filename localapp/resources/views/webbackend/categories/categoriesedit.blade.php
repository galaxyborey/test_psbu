...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="{{url('/adminsite/catemenu/create')}}">{{ trans('template.category') }}</a>
			<i class="fa fa-circle"></i>
		</li>
		<li>
	...
	<div class="col-md-12">
		<div class="portlet light portlet-fit portlet-form bordered">
			...
			<div class="portlet-body form-body">
				<form class="form-horizontal" method="POST" action="{{url('/adminsite/cateUpdateSave')}}" id="form_validation">
							...
							<div class="col-md-6">
								...
								<div class="form-group{{ $errors->has('name_kh') ? ' has-error' : '' }}">
									...
									<div class="col-md-8">
										...
										<input id="cateid" type="hidden" class="form-control" name="cateid" value="{{ $category->id }}" >
										<input id="name_kh" type="text" class="form-control" name="name_kh" value="{{ $category->name_kh }}" autofocus>
								...
								<div class="form-group{{ $errors->has('ordering') ? ' has-error' : '' }}">
									...
									<div class="col-md-8">
										<input id="ordering" type="ordering" class="form-control" name="ordering" value="{{ $category->ordering }}">
								...
								<div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
									...
									<div class="col-md-8">
										<select name="status" id="status" class="form-control">
		                                    <?php $status = $category->cat_active;?>
								...
								<div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
									...
									<div class="col-md-8">
										<select name="pagecate" class="form-control">
											<option value="0"> {{ trans('template.select') }} </option><?php $catepid = $category->catepageid;?>
										<?php 
          ...
          <div id="myTabContent" class="tab-content">
            <div role="tabpanel" class="tab-pane fade active in" id="tab_content1" aria-labelledby="home-tab">
               <div class="form-group">
                    ...
                    {{ Form::textarea('excerpt', $category->short_body, ['class' => 'form-control']) }}
               </div>
               <div class="form-group">
                    {{ Form::label('body', trans('lang.description')) }}
                    {{ Form::textarea('body', $category->body, ['class' => 'form-control','id' => 'my-editor_en']) }}
               </div>
            </div>
            <div role="tabpanel" class="tab-pane fade" id="tab_content2" aria-labelledby="profile-tab">
              <div class="form-group">
                    {{ Form::label('excerpt_kh', trans('lang.excerpt_kh')) }}
                    {{ Form::textarea('excerpt_kh', $category->short_body_kh, ['class' => 'form-control']) }}
              </div>
              <div class="form-group">
                  {{ Form::label('body', trans('lang.description')) }}
                  {{ Form::textarea('body_kh', $category->bodykh, ['class' => 'form-control','id' => 'my-editor']) }}
              </div>
            </div>
          </div>
        </div>
					...
					<div class="form-actions">
						<div class="row">
							<div class="col-md-offset-4 col-md-6">
								...
								<a href="{{url('/adminsite/catemenu/catelist')}}"><button type="button" id="btnClose" class="btn btn-danger"> <i class="fa fa-times-circle"> </i> <strong> {{trans('template.cancel')}}</strong></button> </a>
							</div>
						</div>
					</div>
				</form>
